    ...
    public void run(String... args) throws Exception {
        ...
        if (objetivoRepo.count() == 0) {
            ...
            objetivoRepo.saveAll(Arrays.asList(
                    new Objetivo(null, "Bajar peso (Déficit)", -500f),
                    new Objetivo(null, "Mantener peso", 0f),
                    new Objetivo(null, "Subir masa muscular (Volumen)", 500f)));
        }
        ...
        if (actividadRepo.count() == 0) {
            ...
            actividadRepo.saveAll(Arrays.asList(
                    new NvActividad(null, "Sedentario (Poco o ningún ejercicio)", 1.1f),
                    new NvActividad(null, "Ligero (Ejercicio 1-3 días/sem)", 1.375f),
                    new NvActividad(null, "Moderado (Ejercicio 3-5 días/sem)", 1.55f),
                    new NvActividad(null, "Intenso (Ejercicio 6-7 días/sem)", 1.725f),
                    new NvActividad(null, "Muy intenso (Ejercicio diario intenso o trabajo físico)", 1.9f)));
        }

        if (tipoAlimentacionRepo.count() == 0) {
            tipoAlimentacionRepo.saveAll(Arrays.asList(
                    new TipoAlimentacion(null, "Omnívora"),
                    new TipoAlimentacion(null, "Vegetariana"),
                    new TipoAlimentacion(null, "Vegana"),
                    new TipoAlimentacion(null, "Keto")));
        }
